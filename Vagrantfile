...
config.vm.define "input_webserver" do |input_webserver|
  # These are options specific to the webserver VM
  input_webserver.vm.hostname = "input-webserver"
...
input_webserver.vm.network "forwarded_port", guest: 80, host: 8081, host_ip: "127.0.0.1"
# We set up a private network that our VMs will use to communicate
# with each other. Note that I have manually specified an IP
# address for our webserver VM to have on this internal network,
# too. There are restrictions on what IP addresses will work, but
...
input_webserver.vm.network "private_network", ip: "192.168.56.14"

# This following line is only necessary in the CS Labs... but that
# may well be where markers mark your assignment.

input_webserver.vm.synced_folder ".", "/vagrant", owner: "vagrant", group: "vagrant", mount_options: ["dmode=775,fmode=777"]
...
input_webserver.vm.provision "shell", path: "build-inputwebserver-vm.sh"
